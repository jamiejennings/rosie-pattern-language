...
ok, eid2 = wapi.new_engine(json.encode{name="hello"})
check(ok)
...
ok, info_js = wapi.inspect_engine(eid)
check(ok)
...
ok, msg = wapi.inspect_engine()
check(not ok)
check(invalid_id(msg))
ok, msg = wapi.inspect_engine("foobar")
check(not ok)
check(invalid_id(msg))

subheading("delete_engine")
check(type(wapi.delete_engine)=="function")
ok, msg = wapi.delete_engine(eid2)
check(ok)
check(not msg)
ok, msg = wapi.delete_engine(eid2)
...
ok, msg = wapi.inspect_engine(eid2)
check(not ok)
check(invalid_id(msg))
check(wapi.inspect_engine(eid), "other engine with same name still exists")

subheading("get_environment")
check(type(wapi.get_environment)=="function")
ok, env_js = wapi.get_environment(eid, json.encode(nil)) -- null
check(ok)
...
check(not ok)
check(invalid_id(msg))
ok, def = wapi.get_environment(eid, json.encode("$"))
...
for _, exp in ipairs{"[0-9]", "[abcdef123]", "[:alpha:]", 
		     "[^0-9]", "[^abcdef123]", "[:^alpha:]", 
		     "[^[a][b]]"} do
   ok, msg = wapi.load_string(eid, 'cs = '..exp)
   check(ok)
   ok, msg = wapi.get_environment(eid, json.encode("cs"))
...
ok, msg = wapi.load_string(eid, '-- comments and \n -- whitespace\t\n\n',
...
ok, msg = wapi.load_string(eid, g)
check(ok)
check(not msg)

ok, def = wapi.get_environment(eid, json.encode("S"))
check(ok)
def = json.decode(def)
...
ok, msg = wapi.clear_environment(eid)
check(not ok)
ok, msg = wapi.clear_environment(eid, "null")	    -- json-encoded arg
check(ok)
check(msg==true)
...
ok, def = wapi.get_environment(eid, json.encode("num"))
check(ok)
def = json.decode(def)
check(def.binding)

ok, msg = wapi.load_file(eid, "$sys/test/undef.rpl")
check(not ok)
...
ok, msg = wapi.load_file(eid, "$sys/test/synerr.rpl")
check(not ok)
...
ok, msg = wapi.load_file(eid, "./thisfile/doesnotexist")
check(not ok)
check(msg:find("cannot open file"))
check(msg:find("./thisfile/doesnotexist"))

ok, msg = wapi.load_file(eid, "/etc")
check(not ok)
...
check(not ok)
check(invalid_id(msg))
results = {wapi.load_manifest(eid, "$sys/test/manifest")}
ok = results[1]
check(ok)
...
ok, msg = wapi.load_manifest(eid, "$sys/test/manifest.err")
check(not ok)
check(msg:find("Error: cannot open file"))

ok, msg = wapi.load_manifest(eid, "$sys/test/manifest.synerr") -- contains a //
check(not ok)
...
-- ok, match, left = wapi.match_using_exp(eid, ".", "A")
-- check(ok)
-- check(left==0)
-- j = json.decode(match)
-- check(j["*"].text=="A")
...
-- ok, match, left = wapi.match_using_exp(eid, '{"A".}', "ABC")
-- check(ok)
-- check(left==1)
-- j = json.decode(match)
-- check(j["*"].text=="AB")
...
-- ok, msg = wapi.load_manifest(eid, "MANIFEST")
-- check(ok)

-- ok, match, left = wapi.match_using_exp(eid, 'common.number', "1FACE x y")
-- check(ok)
-- check(left==3)
-- j = json.decode(match)
...
-- ok, match, left = wapi.match_using_exp(eid, '[:space:]* common.number', "   1FACE")
-- check(ok)
-- check(left==0)
-- j = json.decode(match)
-- check(j["*"].pos==1)
...
ok, msg = wapi.configure_engine(eid)
check(not ok)
check(msg:find("configuration argument not a string"))

ok, msg = wapi.configure_engine(eid, json.encode({expression="common.dotted_identifier",
					  encode="json"}))
check(not ok)
check(msg:find("reference to undefined identifier: common.dotted_identifier"))

ok, msg = wapi.load_file(eid, "$sys/rpl/common.rpl")
check(ok)
ok, msg = wapi.configure_engine(eid, json.encode({expression="common.dotted_identifier",
					  encode=false}))
check(ok)
...
ok, msg = wapi.get_environment(eid, json.encode("hex_only")) -- common.rpl
check(ok)
def = json.decode(msg)
...
ok, msg = wapi.match(eid)
check(not ok)
check(msg:find("input argument not a string"))

ok, msg = wapi.load_manifest(eid, "$sys/MANIFEST")
check(ok)

results = {wapi.match(eid, "x.y.z")}
ok = results[1]
check(ok)
...
ok, msg = wapi.configure_engine(eid, json.encode{expression='common.number', encode=false})
check(ok)
...
check(not ok)
check(invalid_id(msg))
ok, msg = wapi.match_file(eid)
check(not ok)
check(msg:find("bad input file name"))

ok, msg = wapi.match_file(eid, ROSIE_HOME.."/test/test-input")
check(not ok)
check(msg:find("bad output file name"))

ok, msg = wapi.match_file(eid, "thisfiledoesnotexist", "", "")
...
ok, msg = wapi.configure_engine(eid, json.encode{expression=macosx_log1, encode="json"})
check(ok)			    
results = {wapi.match_file(eid, ROSIE_HOME.."/test/test-input", "/tmp/out", "/dev/null")}
ok = results[1]
...
results = {wapi.match_file(eid, ROSIE_HOME.."/test/test-input", "/tmp/out", "/tmp/err")}
ok = results[1]
check(ok)
retvals = json.decode(results[2])
c_in, c_out, c_err = retvals[1], retvals[2], retvals[3]
...
results = {wapi.match_file(eid, ROSIE_HOME.."/test/test-input", "", "/tmp/err")}
...
results = {wapi.match_file(eid, ROSIE_HOME.."/test/test-input", "/tmp/out", "")}
...
ok, msg = wapi.configure_engine(eid, json.encode{encode="color"})
check(ok)
results = {wapi.match_file(eid, ROSIE_HOME.."/test/test-input", "", "/tmp/err")}
...
check(not ok)
check(invalid_id(msg))
ok, msg = wapi.eval(eid)
check(not ok)
check(msg=="Argument error: input argument not a string")

ok, msg = wapi.configure_engine(eid, json.encode{expression=".*//", encode="json"})
check(not ok)
check(msg:find('Syntax error at line 1:'))

ok = wapi.configure_engine(eid, json.encode{expression=".*", encode="json"})
check(ok)
results = {wapi.eval(eid, "foo")}
ok = results[1]
check(ok)
...
ok, msg = wapi.configure_engine(eid, json.encode{expression="[[:digit:]]", encode="json"})
check(ok)
results = {wapi.eval(eid, "foo")}
ok = results[1]
check(ok)
...
ok, msg = wapi.configure_engine(eid, json.encode{expression="[[:alpha:]]*", encode="json"})
check(ok)
results = {wapi.eval(eid, "foo56789")}
ok = results[1]
check(ok)
retvals = {table.unpack(results, 2)}
check(retvals[1])
...
ok, msg = wapi.configure_engine(eid, json.encode{expression="common.number", encode="json"})
check(ok)
results = {wapi.eval(eid, "abc.x")}
ok = results[1]
check(ok)
retvals = {table.unpack(results, 2)}
...
if ok then
 ...
 ok, msg = wapi.configure_engine(eid, json.encode{expression=macosx_log1, encode="json"})
 check(ok)			    
 results = {wapi.eval_file(eid, ROSIE_HOME.."/test/test-input", "/tmp/out", "/dev/null")}
 ok = results[1]
 check(ok, "the macosx log pattern in the test file works on some log lines")
 retvals = json.decode(results[2])
 c_in, c_out, c_err = retvals[1], retvals[2], retvals[3]
...
ok, msg = wapi.eval_file(eid, ROSIE_HOME.."/test/test-input")
check(not ok)
check(msg:find(": bad output file name"))

ok, msg = wapi.eval_file(eid, "thisfiledoesnotexist", "", "")
check(not ok)
