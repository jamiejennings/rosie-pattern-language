...
check(json.decode(def)=="// built-in RPL pattern //")
...
check((json.decode(def):find('bar = foo / %(')), "checking binding defn which relies on reveal_ast")
...
check(ok)
check(json.decode(def)=="alias W = (!w any)", "checking binding defn which relies on reveal_ast")
