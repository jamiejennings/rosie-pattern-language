...
ok, pkg, warning = wapi.load()
check(not ok)
check(pkg:find("not a string", 1, true))
ok, pkg, warning = wapi.load("foo")
check(not ok)
check(pkg:find("Syntax error at line 1: foo"))
ok, pkg, warning = wapi.load('foo = "a"')
check(ok)
...
ok, pkg, msg = wapi.load('bar = foo / ( "1" $ )')
check(ok)
...
ok, pkg, msg = wapi.load('-- comments and \n -- whitespace\t\n\n')
...
ok, pkg, msgs, fullpath = wapi.loadfile(ROSIE_HOME .. "/test/ok.rpl")
check(ok)
check(type(pkg)=="string")
