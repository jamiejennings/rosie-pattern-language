...
m, leftover = e:match(p, "a b")
check(m and m.type=="*")
check(type(leftover)=="number" and leftover==0)
m, leftover = e:match(p, "b b")
check(not m)

p = e:compile('first:{a b, b b}')		    -- 2 args
m, leftover = e:match(p, "a b")
check(not m)
m, leftover = e:match(p, "ab")
check(m and m.type=="*")
check(type(leftover)=="number" and leftover==0)
m, leftover = e:match(p, "b b")
check(not m)
m, leftover = e:match(p, "bb")
check(not m)
