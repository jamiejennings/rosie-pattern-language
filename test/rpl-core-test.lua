...
function check_match(exp, input, expectation, expected_leftover, expected_text)
   ...
   check(ok, "failed call to api.match", 1)
...
check_match('a+', ' b', false)
check_match('!a+', ' b', true, 1, " ")		    -- matches boundary added at top level
check_match('(!a+)', ' b', true, 1, " ")	    -- matches boundary added at top level
...
check_match('(a)*', ' a a   ', true, 6, " ")	    -- matches boundary added at top level
...
subheading("Range with min")

subheading("Range with max")

subheading("Range with min, max")
