...
----------------------------------------------------------------------------------------
heading("Ranges with SAME utf8 encoding length")
----------------------------------------------------------------------------------------

--[[


subheading("Full ranges")
-- 1 byte
--r = Rsame(utf8.char(0x00), utf8.char(0x7F))
r = codepoint_range(0x00, 0x7F)
check_range(r, {"R", 0, 127})
-- 2 bytes
--r = Rsame(utf8.char(0x80), utf8.char(0x7FF))
r = codepoint_range(0x80, 0x7FF)
check_range(r, {"+", {"full ranges", 194, 223, index=2, len=2}})
-- 3 bytes
--r = Rsame(utf8.char(0x800), utf8.char(0xFFFF))
r = codepoint_range(0x800, 0xFFFF)
check_range(r, {"+", {"full ranges", 224, 239, index=2, len=3}})
-- 4 bytes
--r = Rsame(utf8.char(0x10000), utf8.char(0x10FFFF))
r = codepoint_range(0x10000, 0x10FFFF)
check_range(r, {"+", {"full ranges", 240, 244, index=2, len=4}})

subheading("Singletons")
-- 1 byte
--r = Rsame(utf8.char(0x0), utf8.char(0x0))
r = codepoint_range(0x0, 0x0)
check_range(r, {"R", 0, 0})
--r = Rsame(utf8.char(0x7F), utf8.char(0x7F))
r = codepoint_range(0x7F, 0x7F)
check_range(r, {"R", 0x7F, 0x7F})

subheading("Error catching in Rsame")
-- range in wrong order
ok, r = pcall(Rsame, utf8.char(0x8), utf8.char(0x7))
check(not ok)
-- 1 byte and 2 byte sequences as args
ok, r = pcall(Rsame, utf8.char(0x0), utf8.char(0x80))
check(not ok)
ok, r = pcall(Rsame, utf8.char(0x7F), utf8.char(0x80))
check(not ok)
ok, r = pcall(Rsame, utf8.char(0x7F), utf8.char(0xFF))
check(not ok)
-- 2 byte and 3 byte sequences as args
ok, r = pcall(Rsame, utf8.char(0x80), utf8.char(0x800))
check(not ok)
ok, r = pcall(Rsame, utf8.char(0x7FF), utf8.char(0x800))
check(not ok)
ok, r = pcall(Rsame, utf8.char(0x7FF), utf8.char(0x1000))
check(not ok)
-- 3 byte and 4 byte sequences as args
ok, r = pcall(Rsame, utf8.char(0x800), utf8.char(0x10000))
check(not ok)
ok, r = pcall(Rsame, utf8.char(0xFFFF), utf8.char(0x10000))
check(not ok)
ok, r = pcall(Rsame, utf8.char(0xFFFF), utf8.char(0x10FFFF))
check(not ok)

----------------------------------------------------------------------------------------
heading("Ranges with different utf8 encoding lengths")
----------------------------------------------------------------------------------------
subheading("Spanning 1-byte to 4-byte")
-- entire unicode range
--r = R(utf8.char(0x0), utf8.char(0x10FFFF))
r = codepoint_range(0x0, 0x10FFFF)
check_range(r, {"+", 
		{"R", 
		 0, 
		 127}, 
		...
		{"+", 
		 {"full ranges", 
		  ...
		  index=2, 
		  len=4}}})
--r = R(utf8.char(0x0), utf8.char(0x10000))
r = codepoint_range(0x0, 0x10000)
check_range(r, {"+", 
		{"R", 
		 0, 
		 127}, 
		 ...
		 {"*", 
		  ...
		  {"*", 
		   ...
		   {"R", 
		    128, 
		    128}}}}})
--r = R(utf8.char(0x7F), utf8.char(0x10FFFF))
r = codepoint_range(0x7F, 0x10FFFF)
check_range(r, {"+", 
		{"R", 
		 127, 
		 127}, 
...
subheading("Misc codepoint pairs")

print("Need more tests of misc codepoint pairs")

--]]


subheading("Error catching")
