...
local function process_manifest_line(en, line, manifest_path, root)
   ...
   if subs then
      ...
      local filename, msg = common.compute_full_path(path, manifest_path, root)
...
function manifest.process_manifest(en, manifest_filename, root)
   assert(engine.is(en))
   local full_path, manifest_path = common.compute_full_path(manifest_filename, nil, root)
   ...
   while line and success do
      success, messages = process_manifest_line(en, line, manifest_path, root)
